...
- While the legacy shared `app_store` is still in use, public Client registrations use `Disabled` status (awaiting approval) and must not enter the portal until normalized Client queries and RLS are active.
